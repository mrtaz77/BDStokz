...
## HIGH 
- Returns the __maximum__ latest price of all  successful transactions of the stock on its latest transaction date
```sql 
...
## LOW 
- Returns the __minimum__ latest price of all  successful transactions of the stock on its latest transaction date
```sql
...
## CLOSE 
- Returns the latest price of the __last__ successful transaction of the stock on its latest transaction date
```sql 
CREATE OR REPLACE FUNCTION CLOSE ( SYM IN VARCHAR2 ) RETURN NUMBER IS CL NUMBER ( 10, 4 );
BEGIN
		CL := 0.0000;
	SELECT
...
## OPEN
- Returns the latest price of the __first__ successful transaction of the stock on its latest transaction date
```sql
CREATE OR REPLACE FUNCTION OPEN ( SYM IN VARCHAR2 ) RETURN NUMBER IS OP NUMBER ( 10, 4 );
BEGIN
		OP := 0.0000;
	SELECT
